...
public sealed class IsGraphCompatibleEventArgs<TVertex, TEdge> : EventArgs
    ...
    where TVertex : notnull
{
    public IsGraphCompatibleEventArgs(TEdge edge, CondensatedContext<TVertex, TEdge> context)
    {
        Edge = edge;
        Context = context;
    }
    ...
    public CondensatedContext<TVertex, TEdge> Context { get; }
}
    ...
    where TVertex : notnull
{
    ...
    public CondensatedContext<TVertex, TEdge> MergeTwoVertex(TVertex source, TVertex target)
    {
    ...
    public void SplitTwoVertex(CondensatedContext<TVertex, TEdge> context)
    {
        RemoveCluster(context.MergedCluster);
        var sourceCluster = ClusteredGraph.AddCluster();
        foreach (var vertex in context.SourceRemovedVertices)
        {
        ...
        foreach (var vertex in context.TargetRemovedVertices)
        {
        ...
        foreach (var removedEdge in context.SourceRemovedEdges.Concat(context.TargetRemovedEdges))
        {
            ReAddEdges(removedEdge);
        }
    }
    ...
    protected override void InternalCompute()
    {
        ...
        dfsVisitEdge.InitializeEdge += (edge) =>
        {
            ...
            var context = MergeTwoVertex(edge.Source, edge.Target);

            compatible = IsGraphCompatible?.Invoke(this, new(edge, context)) ?? true;

            if (!compatible)
            {
                SplitTwoVertex(context);
            }
        };
...
public record CondensatedContext<TVertex, TEdge>(List<TVertex> SourceRemovedVertices, List<TEdge> SourceRemovedEdges, List<TVertex> TargetRemovedVertices, List<TEdge> TargetRemovedEdges, ClusteredBidirectionalGraph<TVertex, TEdge> MergedCluster)
    where TEdge : class, IEdge<TVertex>
    where TVertex : notnull
{
}
